...
1. Compute invariances to synthetic data augmentations, e.g., rotations, horizontal / vertical flips, shears, hue transforms, etc. For example, to compute the invariance of CheXpert to horizontal flip augmentations for the MoCo pretrained model, run
```
...
2. Compute invariances to different views of the same image. This is only compatible with the CheXpert and EyePACS datasets, which both contain multiple images from different views of the same patient. For example, to compute the multi-view invariance of EyePACS, with the model SwAV, run:
```
